...
class Guard
{
    ...
    {
        // logger(implode(' | ', class_uses_recursive(get_class($tokenable))));
    ...
    protected function isValidAccessToken($accessToken): bool
    {
        ...
        // logger('expiration ' . $this->expiration);
        // logger('created_at->timestamp ' . $accessToken->created_at->timestamp);
        // logger('now-sub-timestamp ' . now()->subSeconds($this->expiration  * 60)->timestamp);
        // logger($accessToken->created_at->gt(now()->subSeconds($this->expiration * 60)) ? 'Y-greater' : 'N-greater');
        // logger(!$accessToken->expires_at->isPast() ? 'Y-isPast' : 'N-isPast');
        // logger($this->hasValidProvider($accessToken->tokenable) ? 'Y-Provider' : 'N-Provider');
        // logger($isValid ? 'Y-1' : 'N-1');

        if (is_callable(Sanctum::$accessTokenAuthenticationCallback)) {
            $isValid = (bool) (Sanctum::$accessTokenAuthenticationCallback)($accessToken, $isValid);
        }
        // logger($isValid ? 'Y-2' : 'N-2');
